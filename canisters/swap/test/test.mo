...
actor class TestSwap(
    ...
    let demoCanister : TokenActor = actor(Principal.toText(demo_id));
    ...
    public func tests(): async Text {
        ...
        log_info("- - - - - - - - - - - - - - - - - - ");
        log_info("c) create new pair (wicp demo)");
        result := await testCreatePair(wicp_id, demo_id);
        if (result) {
            log_info("! ! ! test fail");
            fail_count += 1;}
        else {pass_count += 1;};
        ...
        log_info("- - - - - - - - - - - - - - - - - - ");
        log_info("c) addLiquidity to nonexitent pair(wicp demo)");
        result := await testAddLiquidity(wicp_id, demo_id, 1000_00000000, 50000_00000000, 0, 0, Time.now()*2);
        if (result) {
            log_info("! ! ! test fail");
            fail_count += 1;}
        else {pass_count += 1;};
        ...
        log_info("- - - - - - - - - - - - - - - - - - ");
        log_info("b): removeLiquidity with nonexitent pair(wicp, demo) ");
        result := await testRemoveLiquidity(wicp_id, demo_id, lp_balance, 0, 0, Principal.fromActor(this), Time.now()*2);
        if (result) {
            log_info("! ! ! test fail");
            fail_count += 1;}
        else {pass_count += 1;};
        ...
        log_info("a): withdraw demo (not exist currently)");
        result := await testWithdraw(demo_id, 1000_00000000);
        if (result) {
            log_info("! ! ! test fail");
            fail_count += 1;}
        else {pass_count += 1;};
        ...
        result := await testWithdraw(demo_id, 1);
        if (result) {
            log_info("! ! ! test fail");
            fail_count += 1;}
        else {pass_count += 1;};
