...
shared(msg) actor class Swap(owner_: Principal, swap_id: Principal) = this {
    ...
    type ICRCTransferError = {
        ...
        #CustomError:Text; // custom error for demoSwap logic
    };
    type TokenTxReceipt = {
        #Ok: Nat;
        #Err: Errors;
    ...
    */
    ...
    // e.g. tokenA's transfer fee is 1 when added to demoSwap, in demoSwap's record the fee is 1,
    // later tokenA's transfer fee is changed to 2, if demoSwap is not up to date, will cause
    // demoSwap to lose money when users withdraw tokenA from demoSwap
